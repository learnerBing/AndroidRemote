...
class LanRelayHandler(SimpleHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args) -> None:
        print(f"[lan-test] {self.address_string()} {fmt % args}")
    ...
    def _handle_test_register_post(self, data: dict[str, Any]) -> None:
        ...
        print(f"[lan-test] browser registered code {code}")
        self._json(200, {"ok": True})
    ...
    def _handle_sdp_post(self, data: dict[str, Any]) -> None:
        ...
        print(f"[lan-test] SDP {sdp_type} session={session_id[:8]}… bytes={len(sdp)}")
        self._json(200, {"ok": True})
    ...
    def _handle_sdp_offer_get(self, query: dict[str, str]) -> None:
        ...
        if not offer:
            print(f"[lan-test] offer GET 204 session={session_id[:8]}… (start iPhone broadcast)")
            self.send_response(204)
            self.end_headers()
            return
        ...
        with lock:
            ...
            if cand_str:
                ...
                if not any(c.get("candidate") == cand_str for c in existing):
                    ...
                    print(f"[lan-test] ICE {side} +1 session={session_id[:8]}… total={len(session[key])}")
        self._json(200, {"ok": True})
    ...
    def _handle_status_post(self, data: dict[str, Any]) -> None:
        ...
        print(f"[lan-test] status → {state} session={session_id[:8]}…")
        self._json(200, {"ok": True})
